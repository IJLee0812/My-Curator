...
function AccordionSection({
  ...
  return (
    <div className="border border-line rounded-xl overflow-hidden">
      ...
      {open && <div className="bg-surface-2 px-4 py-3">{children}</div>}
    </div>
  );
}
...
function KV({ k, v }: { k: string; v: React.ReactNode }) {
  return (
    <div className="flex items-start gap-3 py-1.5 border-b border-line/50 last:border-0">
      <span className="text-xs text-muted w-36 shrink-0 pt-0.5">{k}</span>
      <span className="text-xs text-ink break-all">{v}</span>
    </div>
  );
}
...
export default function DnaAccordion({ dna }: { dna: ScenarioDNA | null }) {
  if (!dna) {
    return (
      <div className="border border-line rounded-xl p-4 text-xs text-muted">
  ...
  return (
    <div className="space-y-2">
      {dna.scene_description && (
        <div className="border border-line rounded-xl bg-surface-2 px-4 py-3">
          <div className="text-xs font-medium text-accent mb-1.5">Scene Description</div>
      ...
      <AccordionSection
        ...
        color="bg-blue-500/10 text-blue-700 dark:text-blue-300 border-b border-blue-500/20"
        defaultOpen
      >
      ...
      <AccordionSection
        ...
        color="bg-purple-500/10 text-purple-700 dark:text-purple-300 border-b border-purple-500/20"
        defaultOpen
      >
        <div className="space-y-1 mb-3">
      ...
      <AccordionSection
        ...
        color="bg-orange-500/10 text-orange-700 dark:text-orange-300 border-b border-orange-500/20"
        defaultOpen
      >
        {actors.length === 0 ? (
          <p className="text-xs text-faint italic">No dynamic actors detected</p>
        ) : (
          <div className="space-y-3">
            {actors.map((actor, i) => (
              <div key={i} className="bg-surface rounded-lg p-3 border border-line">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs font-mono text-orange-700 dark:text-orange-300">[{i}]</span>
                  <span className="text-xs font-semibold text-ink">
                    {actor.actor_class.replace(/_/g, " ")}
                  </span>
                  {actor.grounded_by_yolo26 && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-green-500/15 text-green-600 dark:text-green-400 border border-green-500/25">
                      YOLO26 ✓
                    </span>
                  )}
                </div>
      ...
      <AccordionSection
        ...
        color="bg-pink-500/10 text-pink-700 dark:text-pink-300 border-b border-pink-500/20"
        defaultOpen
      >
        <div className="space-y-1 mb-3">
      ...
      {dna.confidence && (
        <AccordionSection
          title="Confidence"
          color="bg-accent/10 text-accent border-b border-accent/20"
        >
          <div className="space-y-3">
            <div>
            ...
            <div>
              <div className="text-xs text-muted mb-1">Scout Agreement</div>
              <ConfidenceBar value={dna.confidence.scout_agreement} />
            </div>
            {dna.confidence.hallucination_flags?.length > 0 && (
              <div>
                <div className="text-xs text-amber-600 dark:text-amber-400 mb-1">⚠ Hallucination flags</div>
      ...
      {provenance && (
        <AccordionSection
          title="Provenance"
          color="bg-faint/10 text-muted border-b border-line/20"
        >
          <div className="space-y-1">
